...
export const bisecLeft: TBisecWidthDirection = (list, comparator) =>
  bisec(list, comparator, (result, index, counter) => {
    if (result >= 0) {
...
export const bisecRight: TBisecWidthDirection = (list, comparator) =>
  bisec(list, comparator, (result, index, counter) => {
    if (result > 0) {
      return { ...counter, end: index }
    } else {
